...
df_list = [df_194,df_196,df_198,df_200,df_202,df_204,
           df_206,df_208,df_209,df_210,df_212,
           df_214,df_216,df_218,df_220,df_221,df_222,
           df_223,df_224,df_225,df_226,df_227,df_228]
...
print(test2)
print((test2+test)/2)
df = df.groupby(np.arange(len(df))//40).mean()
print(df)
print(df.iloc[-1:,0])
#df.drop(df.tail(1).index,inplace=True)

#cmap = sns.diverging_palette(220, 20, as_cmap=True, n=2)
def corr_mat(df):
    cm = df.iloc[:,0:316].corr()
    sns.heatmap(cm, cmap = LinearSegmentedColormap.from_list(
    name='blwrd', 
    colors=['blue','white','white','white', 'white','white','red']
) , vmin=-1, vmax=1, cbar_kws=dict(ticks=[-1,-0.75,-0.5,-0.25,0,0.25,0.5,0.75,1]) )

def cov_mat(df):
    cm = df.iloc[:,0:316].cov()
    sns.heatmap(cm, cmap = LinearSegmentedColormap.from_list(
    name='blwrd', 
...
#df = pd.read_csv("ET_images/198-ert.csv", header = None, skiprows=2).iloc[:, 2:]
#test5 = df.iloc[0:40,0].mean()
#print('First 40 mean:',test5)
#test = df.iloc[-40:,0].mean()
#print('Last 40 mean:',test)
#test4 = df.iloc[-30:,0].mean()
#print('Last 30 mean:',test4)
#test2 = df.iloc[-20:,0].mean()
#print('Last 20 mean:',test2)
#test3 = df.iloc[-10:,0].mean()
#print('Last 10 mean:',test3)
#df = pd.read_csv("ET_images/198-ert.csv", header = None, skiprows=12).iloc[:, 2:]
#df = df.groupby(np.arange(len(df))//40).mean()
...
experiment = 228
df2 = pd.read_csv('ET_images/{}-ert.csv'.format(experiment), header = None, skiprows=2).iloc[:, 2:]


im1 = df2.iloc[:,:316]
im2 = df2.iloc[:,316:]
'''
for pixel in [4,12,24,39,55,72,90,109,129,149,169,189,209,228,246,263,279,294,306,314]:
    plot_acf(im1.iloc[:,pixel], title = '{}-ert, Pixel: {}'.format(experiment,pixel), lags = 20)
    print(acf)
'''

matrix = np.zeros((len(im1)//40,316))

df3 = pd.DataFrame(df2)

for j in range(len(im1)//40):
    for i in range(316):
        if math.isnan(acf(im1.iloc[(j*40):(40*(j+1)),i],fft=False)[5]) == True:
            matrix[j,i] = 0
        else:
            matrix[j,i] = acf(im1.iloc[(j*40):(40*(j+1)),i],fft=False)[5]
         
norm_mat = np.zeros((len(im1)//40,316))
for i in range(0,250):
    for j in range(316):
        if math.isnan(matrix[i,j] - min(matrix[i,]) )/(max(matrix[i,]) - min(matrix[i,])) == True:
            norm_mat[i,j] = 0
        else:
            norm_mat[i,j] = (matrix[i,j] - min(matrix[i,]) )/(max(matrix[i,]) - min(matrix[i,]))
            

       
'''
print(norm_mat)
plot_acf(im1.iloc[:,2],fft=False)
plot_acf(im1.iloc[:,3],fft=False)
plot_acf(im1.iloc[:,4],fft=False)
plot_acf(im1.iloc[:,5],fft=False)
plot_acf(im1.iloc[:,6],fft=False)
plot_acf(im1.iloc[:,7],fft=False)
plot_acf(im1.iloc[:,8],fft=False)
plot_acf(im1.iloc[:,9],fft=False)
